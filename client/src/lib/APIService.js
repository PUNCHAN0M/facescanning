...
export class APIService {
  ...
  /**
   * Extract a 512-d face embedding vector from an image using the SERVER-SIDE pipeline.
   * (detect + align + ArcFace — mirrors ImageToVectorService but runs on the server)
   *
   * @param {File|Blob} file       - Face image file
   * @param {string}   [modelKey] - Embedding model key ("w600k_r50", "w600k_mbf", "r100")
   * @returns {Promise<{vector: number[], dims: number, model: string}>}
   */
  async imageToVector(file, modelKey) {
    if (!file) {
    ...
    const params = modelKey ? `?model=${encodeURIComponent(modelKey)}` : "";
    return await this.request(`/image-to-vector${params}`, {
      method: "POST",
      body: formData,
    });
  }
