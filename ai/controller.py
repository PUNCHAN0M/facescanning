...
# ═══════════════════════════════════════════════════════
#  Embedding Router  (image → vector only, no search)
# ═══════════════════════════════════════════════════════

embedding_router = APIRouter(tags=["embedding"])


@embedding_router.post("/image-to-vector")
async def image_to_vector(
    file: UploadFile,
    model: str = Query(None, description="Embedding model key (w600k_r50, w600k_mbf, r100)"),
):
    """
    Extract a 512-d L2-normalised face embedding vector from an uploaded image.

    Pipeline (server-side): image → detect face → align → ArcFace → float32[512]

    The returned vector can be used to:
      • Store a member's embedding:  POST /organize/{name}/member/{person}/upload-with-vector
      • Rebuild the FAISS index:     POST /organize/{name}/rebuild?source=face-vector
      • Direct FAISS search:         POST /organize/{name}/search_vector

    Returns:
        JSON with "vector" (list of 512 floats) and metadata.
    """
    import numpy as np
    ...
    image_bytes = await file.read()
    embedding = _face_recognition_service.extract_vector_from_image(
        image_bytes, model_key=model
    )

    if embedding is None:
        raise HTTPException(
            status_code=422,
            detail="No face detected in the image or embedding extraction failed",
        )
